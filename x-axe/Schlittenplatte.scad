...
bohrungen = [
  ...
  [21.0,54.9,-1,4,10,0], [51.0,54.9,-1,4,10,0], [18.35,39.85,-1,4,10,0], [55.25,39.85,-1,4,10,0], //5. Löcher
...
//import("CarriageRouletteFront2020.stl", convexity=3);
module Schlittenplatte(dicke = 8.35, baender = true, centered = [0,0,0])
{
  translate([-GPbreite/2*centered[0],-GPhoehe/2*centered[1],-GPhoehe/2*centered[2]])
    difference()
    {
      union()
      {
...
module bohrungsmaske( h, baender = true, centered = [0,0,0],luft = .2, screwtype = -1)
{
  translate([-GPbreite/2*centered[0],-GPhoehe/2*centered[1],-GPhoehe/2*centered[2]])      for(pos = bohrungen)
      {
        ...
        if(len(pos) == 6)
        {
          ...
          {
            if(screwtype <0 )
              cylinder(d=pos[3]+luft, h=pos[4]+h, $fn=90);
            else 
              translate([0,0,h]) rotate([180,0,0])
                metrische_schraube_schablone(typ = screwtype, mass= pos[3],laenge = h, toleranz = luft);
            if(baender)
            {
              translate([0, 0, dicke-pos[3]/2]) metrische_mutter_schablone(pos[3],startw = pos[5], toleranz = 2*luft); //creates a M5 nut, rotated by 30deg and with an added .1 width
            }
          }
        }
        else if(len(pos) == 5)
        {
          translate([pos[0], pos[1], pos[2]]) cylinder(d=pos[3]+luft, h=pos[4], $fn=90);
        }
        else if(len(pos) == 4)
        {
          translate([pos[0], pos[1], pos[2]]) cylinder(d=pos[3]+luft, h=10+h, $fn=90);
        }
        else
        {
          color("blue")
            translate([pos[0], pos[1], -1]) cylinder(d=pos[2]+luft, h=10+h, $fn=90);
        }
      }
}//module bohrungsmaske( h) 
