...
module metrische_t_mutter(mass,startw = 30)
{
  ...
  // M versatz d   mutter laenge breite
  //[6, 1.5,   8,   6,    16.5,  7.9],
  // 0  1      2    3      4      5
  MUTTER  =3; LAENGE = 4; BREITE = 5;
  nase = (data[3] - data[1]) *0.7;
  p = [
    [data[2]/2,0],
    [data[2]/2,data[1]],
    [data[4]/2,data[1]],
    ...
    translate([0,0,data[3]])
      ...
      difference()
      {
         union()
         {
        //translate([0,0,data[MUTTER]/2])
//color("blue")
                    //cube([data[LAENGE],data[BREITE],data[MUTTER]], center=true);
        translate([0,data[5]/2-0.05*data[BREITE],0])
          rotate([90,0,0])
          scale([0.9,0.9,0.9])
          minkowski()
          {
            difference()
            {
              linear_extrude(height=data[BREITE])
                polygon(p);
                union()
                {
              translate([data[LAENGE]/2+data[BREITE]/10,0.3*data[BREITE],-data[BREITE]/10])rotate([0,-90,0])rotate([90,0,0])fillet(r=data[MUTTER],h=2*data[MUTTER]);
              translate([-data[LAENGE]/2-data[BREITE]/10,0.3*data[BREITE],1.1*data[BREITE]])rotate([0,90,0])rotate([90,0,0])fillet(r=data[MUTTER],h=2*data[MUTTER]);
              translate([data[MUTTER]/2+1,-data[MUTTER]/2+data[1],0])rotate([0,-90,0])rotate([90,0,0])fillet(r=data[MUTTER]/2+1,h=data[MUTTER]);
             color("magenta")translate([-(data[MUTTER])/2-1,-data[1],(data[MUTTER])+1.9])rotate([0,90,0])rotate([90,0,0])fillet(r=data[MUTTER]/2+1,h=data[MUTTER]);
                }
            }
            sphere(d=1.0,$fn=100);
          }
